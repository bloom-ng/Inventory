<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @lang('crud.item_locations.index_title')
        </h2>
    </x-slot>
                ...
                <div class="block w-full overflow-auto scrolling-touch">
                    <table class="w-full max-w-full mb-4 bg-transparent">
                        ...
                        <tbody class="text-gray-600">
                            @forelse($itemLocations as $itemLocation)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-left">
                                    {{ $itemLocation->item->name ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                    {{ $itemLocation->department->name ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                ...
                                >
                                    <div
                                        ...
                                        "
                                    >
                                        @can('update', $itemLocation)
                                        <a
                                            href="{{ route('item-locations.edit', $itemLocation) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        @endcan @can('view', $itemLocation)
                                        <a
                                            href="{{ route('item-locations.show', $itemLocation) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        @endcan @can('delete', $itemLocation)
                                        <form
                                            action="{{ route('item-locations.destroy', $itemLocation) }}"
                                            method="POST"
